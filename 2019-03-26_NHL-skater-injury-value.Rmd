...
WAR, first developed by sabermetricians in baseball circles, is an evaluative metric intended to approximate a player’s impact in units of wins added, measured against what a replacement level player is expected to offer in the same circumstances[^8]. Recently, this framework has been applied in the context of the NHL by Thomas et al. (2014)[^9], Sprigings (2016)[^10], Perry (2017)[^11], and Solberg and Solberg[^12]. Replacement level is not uniquely defined, but it is commonly equated to players earning a league-minimum salary.  

In this paper, I posit that player value should be considered when evaluating injuries. A team’s most productive players tend to play the most, and as such are subjected to a higher degree of injury risk, both in terms of overuse injuries and exposure to deleterious on-ice events such as hits and blocked shots. The NHL is also to some extent a meritocracy, and it is common for elite players to receive special or preferential medical treatment that should, in theory, mitigate these injury risks to some extent. An efficient medical staff is one that is able to identify risk factors and reduce training load and exposure prior to injury, while helping players who have been injured return to action more quickly, ultimately improving the player's availability and total output. In developing more nuanced metrics for evaluating value lost to injury, we’re able to more accurately identify teams with recurring injury problems, which is the first step towards developing appropriate solutions to these problems.
